...
function get_company_levy($company_id){
	...
	function send_sms($number, $ref){
		...
		$getString .="orig=ctesential&";
		$getString .="msg=".urlencode("Thank you for choosing TAP. Your booking with Ref# ".$ref." is confirmed, for arrival & departure procedure please check your email.")."&";
		$getString .="pass=oYq4FMOuk5KtJdxz&";
		$getString .="uid=api-F6dOc7NBGioD&";
    ...
    function getBookingByRefferenceNumber($reffNo) {
        ...
        switch ($reffParts[0]) {
            case 'CTP':
				...
				//echo "SELECT * FROM ".$db->prefix."booking WHERE referenceNo = '".$reffNo."'";
                break;
...
$mail->IsSMTP();
		...
		$mail->SMTPSecure = "tls";
		$mail->Port     = 587;  
		$mail->Username = 'tap@travelairportplus.co.uk';
		$mail->Password = 'qZY6>8%2';
		...
		$mail->Mailer   = "smtp";
		$mail->AddReplyTo('no-reply@travelairportplus.com', 'TAP');
		$mail->SetFrom('tap@travelairportplus.co.uk', 'Travel Airport Plus');
		...
		$mail->AddAddress('record@travelairportplus.co.uk', $touser);
		if($files != ""){
	...
	function email_signUP($customer_id,$password){
		...
		$from = "noreply@travelairportplus.co.uk";
		$from_name = "Travel Airport Plus";
	...
	$vehicle_row = $db->get_row("SELECT * FROM " . $db->prefix . "vehicle WHERE bookingId = '$token' order by id DESC limit 1"); // avoids duplicate if any old record exists
	...
	if(!empty($status)){
        ...
        $message = str_replace("[reg]",$vehicle_row['registration'],$message);
        $message = str_replace("[make]",$vehicle_row['make'],$message);
        $message = str_replace("[model]",$vehicle_row['model'],$message);
        $message = str_replace("[booktime]",$users['createdate'],$message);
        $message = str_replace("[color]",$vehicle_row['color'],$message);
