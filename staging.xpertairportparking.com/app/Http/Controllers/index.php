...
if($from == 'Parking 4 You <noreply@parking4you.co.uk>' || $from == 'Parking 4 You <noreply@compareparkingdeals.co.uk>'  && strpos($from, 'Parking 4 You') !== false && strpos($subject, 'Ammend') === false   && strpos($subject, 'Cancelled') !== false){
...
if ($data['departDate'] && $data['returnDate']) {
  ...
  if ( $from == 'noreply@smartparkingdeals.uk' || strpos($from, 'Compare Airport Parking Deals') !== false || stripos($from,'Smart Parking Deals') !==false)  {
        // $parse_email_body = strip_tags($body);
...
$data['cancelRequest'] = 1;
 ...
 if (strpos($from, 'no-reply@cheapdealcenter.com') !== false || strpos($from, 'Cheap Deal Center <no-reply@cheapdealcenter.com>') !== false) {
        ...
        // Convert the body to UTF-8 and clean it up
        $body = mb_convert_encoding($body, 'UTF-8', 'auto');
        $body = preg_replace('/\xC2\xA0/', ' ', $body);
        $parse_email_body = strip_tags($body);
        $parse_email_body = html_entity_decode($parse_email_body, ENT_QUOTES, 'UTF-8');
        ...
        $parse_email_body = preg_replace('/.*?(Booking Confirmation Details:)/s', '$1', $parse_email_body);
    
        // Remove 'Booking Confirmation Details:' as per the original code
        $parse_email_body = str_replace('Booking Confirmation Details:', '', $parse_email_body);

        // Define regex patterns to match each field
        $pattern = [
            'reference_number' => '/Reference Number:\s?([A-Za-z0-9\-]+)/',
            'name' => '/Customer Name\s?:?\s?([A-Za-z\s]+)(?=\s?Contact Number|$)/',
            'contact_number' => '/Contact Number\s?(\d{11})/',
            'product_name' => '/Product Name\s?([A-Za-z\s\-]+)/',
            'product_booked_with' => '/Product Booked With\s?([A-Za-z\s\-]+)/',
            'product_code' => '/Product Code\s?([A-Za-z0-9\-]+)/',
            'parking_type' => '/Parking Type\s?([A-Za-z\s\-]+)/',
            'airport' => '/Airport\s?([A-Za-z\s]+)/',
            'departure_terminal' => '/Departure Terminal\s?([A-Za-z0-9\s\-]+)(?=\s+Number Of days)/',  // Updated regex
            'return_terminal' => '/Return Terminal\s?([A-Za-z0-9\s\-]+)(?=\s+Number Of days)/',
            'number_of_days' => '/Number Of days\s?(\d+)/',
            'departure_datetime' => '/Departure Date \/ Time\s?([0-9\-: ]+)/',
            'return_datetime' => '/Return Date \/ Time\s?([0-9\-: ]+)/',
            'booking_datetime' => '/Booking Date \/ Time\s?([0-9\-: ]+)/',
            'return_flight_no' => '/Return Flight No\s?([A-Za-z0-9]+)/',
            'vehicle_registration' => '/Vehicle Registration No\s?([A-Za-z0-9]+)/',
            'vehicle_model' => '/Vehicle Model\s?([A-Za-z0-9]+)/',
            'vehicle_make' => '/Vehicle Make\s?([A-Za-z]+)/',
            'vehicle_color' => '/Vehicle Color\s?([A-Za-z]+)/',
            'amount_paid' => '/Amount Paid\s?(\d+\.\d{2})/'
        ];
        ...
        // Correcting the structure of the data array
        $data = [
            'referenceNo' => $data['reference_number'] ?? '', // Use $data here
            ...
            'deprTerminal' => preg_replace('/Terminal.*?Terminal/i', '', $data['departure_terminal'] ?? ''),
        ...
        $data['companyId'] = 3; // Assuming fixed value
        $data['booking_status'] = 'Completed';
        $data['booking_action'] = 'Booked';
        $data['payment_status'] = 'success';
        $data['traffic_src'] = 'Agent';
        ...
        // Return the final array with the extracted data
        return $data;
