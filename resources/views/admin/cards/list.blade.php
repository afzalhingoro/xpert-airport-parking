...
@section('content')
    <div class="page-content">
        ...
        <div class="col-xs-12">
            ...
            <div class="row">
                <div class="col-md-12">
                    <form action="{{ route('print-card') }}" method="get" class="form-inline mb-10">
                        ...
                        <div class="form-group">
                            ...
                            <a href="{{ route('print-card') }}" class="btn btn-primary btn-sm">Reset</a>
                        </div>
                    </form>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <a target="_blank"
                                href='{{ route('print_card_pdf') }}?search={{ Request::get('search') }}&start_date={{ Request::get('start_date') }}&end_date={{ Request::get('end_date') }}'
                                class="btn btn-primary btn-sm pull-right printBtn"><i class="fa fa-file-pdf-o"></i> Print</a>
                        </div>
                    </div>
                </div>
                <div class="row">
                                        ...
                                        <tbody>
                                            @foreach ($bookings as $booking)
                                                @if ($booking['booking_status'] == 'Completed')
                                                    ...
                                                    @endphp
                                                    <tr>
                                                        <td>{{ date('H:i', strtotime($booking['departDate'])) }}</td>
                                                        <td>Agent</td>
                                                        <td>{{ $booking['referenceNo'] }}</td>
                                                        <td>{{ $fullname }}</td>
                                                        <td>{{ $booking->phone_number }}</td>
                                                        <td>{{ date('d-m-Y', strtotime($booking['departDate'])) }}</td>
                                                        <td>{{ $booking->make }}</td>
                                                        <td>{{ $booking->model }}</td>
                                                        <td>{{ $booking->registration }}</td>
                                                        <td>{{ $booking->returnFlight }}</td>
                                                        <td>
                                                            <button class="btn btn-primary btn-xs"
                                                                onclick="javascript:void
															window.open('{{ url('admin/show_card', $booking['id']) }}','1374577131540','toolbar=0,menubar=0,location=0,status=1,scrollbars=1,resizable=1,left=0,top=0');
															return false;"
                                                                type="button">
                                                                SHOW CARDS
