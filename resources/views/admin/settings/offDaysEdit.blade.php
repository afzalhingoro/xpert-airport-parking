...
@php
            $offTypeData = $offDays->off_type;
            $encodedData = json_encode($offTypeData);

        @endphp
        <div class="row">

            <form method="POST" action="{{ route('offDaysUpdate') }}">
                @csrf
                <div class="row">
                    <div class="col-md-4 col-sm-12">
                        <div class="form-group">
                            ...
                            <select name="off_day_type" id="off_day_type" class="form-control input-sm select-wid"
                                ...
                                <option value="Admin" @if ($offDays->off_type == 'Admin') selected @endif>Admin</option>
                                <option value="Company" @if ($offDays->off_type == 'Company') selected @endif>Company</option>
                            </select>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="admins" class="form-control input-sm">
                                ...
                                @foreach ($admins as $admin)
                                    <option @if ($offDays->admin_id == $admin->id) {{ "selected='selected'" }} @endif
                        ...
                        <div class="form-group">
                            ...
                            <select name="companies" class="form-control input-sm select-wid">
                                ...
                                @foreach ($companies_dlist as $company)
                                    <option @if ($offDays->company_id == $company->id) {{ "selected='selected'" }} @endif
                ...
                <div class="row">
                    <div class="col-md-4 col-sm-12">
                        <div class="form-group">
                            <!--OFF Dates<br>-->
                            <textarea type="text" name="off_dates" class="form-control input-sm" id="off_dates" placeholder="Add Dates">{{ $offDays->off_days }}</textarea>
                ...
                <div class="row">
                    <div class="col-md-4 col-sm-12">
                        <div class="form-group">
                            <input type="hidden" name="offDayId" value="{{ $offDays->id }}">
...
<script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            var selectedOption = <?php echo $encodedData; ?>;
