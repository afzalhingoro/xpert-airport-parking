...
print_r(strpos($body,'notifications@meetandgreetreservations.net')); die();
	    if(strpos($from == 'M&G Reservations') == true && strpos($body,'notifications@meetandgreetreservations.net') == true && strpos($body,'Amendment') == true && strpos($body,'Cancelled') == false){
        $parse_email_body = strip_tags($body);
        $parse_email_body = htmlentities($parse_email_body, ENT_QUOTES, 'utf-8');
        $parse_email_body = html_entity_decode($parse_email_body);
        $parse_email_body = trim(preg_replace('/\s\s+/', ' ', $parse_email_body)); 
